...
This document is an informal standard and replaces the [id3v2.2.0](https://id3.org/id3v2-00) standard. The informal standard is released so that implementors could have a set standard before a formal standard is set. The formal standard will use another version or revision number if not identical to what is described in this document. The contents in this document may change for clarifications but never for added or altered functionality.
...
1.  [ID3 tag version 2.3.0](#ID3_tag_version_2.3.0)
    1.  [Status of this document](#Status_of_this_document)
    2.  [Abstract](#Abstract)
2.  [Conventions in this document](#Conventions_in_this_document)
3.  [ID3v2 overview](#ID3v2_overview)
    1.  [ID3v2 header](#ID3v2_header)
    2.  [ID3v2 extended header](#ID3v2_extended_header)
    3.  [ID3v2 frame overview](#ID3v2_frame_overview)
        1.  [Frame header flags](#Frame_header_flags)
    4.  [Default flags](#Default_flags)
4.  [Declared ID3v2 frames](#Declared_ID3v2_frames)
    1.  [Unique file identifier](#Unique_file_identifier)
    2.  [Text information frames](#Text_information_frames)
        1.  [Text information frames - details](#Text_information_frames_-_details)
        2.  [User defined text information frame](#User_defined_text_information_frame)
    3.  [URL link frames](#URL_link_frames)
        1.  [URL link frames - details](#URL_link_frames_-_details)
        2.  [User defined URL link frame](#User_defined_URL_link_frame)
    4.  [Involved people list](#Involved_people_list)
    5.  [Music CD identifier](#Music_CD_identifier)
    6.  [Event timing codes](#Event_timing_codes)
    7.  [MPEG location lookup table](#MPEG_location_lookup_table)
    8.  [Synchronised tempo codes](#Synchronised_tempo_codes)
    9.  [Unsychronised lyrics/text transcription](#Unsychronised_lyrics.2Ftext_transcription)
    10.  [Synchronised lyrics/text](#Synchronised_lyrics.2Ftext)
    11.  [Comments](#Comments)
    12.  [Relative volume adjustment](#Relative_volume_adjustment)
    13.  [Equalisation](#Equalisation)
    14.  [Reverb](#Reverb)
    15.  [Attached picture](#Attached_picture)
    16.  [General encapsulated object](#General_encapsulated_object)
    17.  [Play counter](#Play_counter)
    18.  [Popularimeter](#Popularimeter)
    19.  [Recommended buffer size](#Recommended_buffer_size)
    20.  [Audio encryption](#Audio_encryption)
    21.  [Linked information](#Linked_information)
    22.  [Position synchronisation frame](#Position_synchronisation_frame)
    23.  [Terms of use frame](#Terms_of_use_frame)
    24.  [Ownership frame](#Ownership_frame)
    25.  [Commercial frame](#Commercial_frame)
    26.  [Encryption method registration](#Encryption_method_registration)
    27.  [Group identification registration](#Group_identification_registration)
    28.  [Private frame](#Private_frame)
5.  [The unsynchronisation scheme](#The_unsynchronisation_scheme)
6.  [Copyright](#Copyright)
7.  [References](#References)
8.  [Appendix](#Appendix)
    1.  [Appendix A - Genre List from ID3v1](#Appendix_A_-_Genre_List_from_ID3v1)
9.  [Author's Address](#Author.27s_Address)
...
The first criterion is met by the simple fact that the [MPEG](https://id3.org/id3v2.3.0#MPEG) decoding software uses a syncsignal, embedded in the audiostream, to 'lock on to' the audio. Since the ID3v2 tag doesn't contain a valid syncsignal, no software will attempt to play the tag. If, for any reason, coincidence make a syncsignal appear within the tag it will be taken care of by the 'unsynchronisation scheme' described in [section 5](https://id3.org/id3v2.3.0#sec5).
...
The ID3 tag described in this document is mainly targeted at files encoded with [MPEG](https://id3.org/id3v2.3.0#MPEG)\-1/2 layer I, [MPEG](https://id3.org/id3v2.3.0#MPEG)\-1/2 layer II, [MPEG](https://id3.org/id3v2.3.0#MPEG)\-1/2 layer III and [MPEG](https://id3.org/id3v2.3.0#MPEG)\-2.5, but may work with other types of encoded audio.
...
Bit 7 in the 'ID3v2 flags' indicates whether or not unsynchronisation is used (see [section 5](https://id3.org/id3v2.3.0#sec5) for details); a set bit indicates usage.

b - Extended header

The second bit (bit 6) indicates whether or not the header is followed by an extended header. The extended header is described in [section 3.2](https://id3.org/id3v2.3.0#sec3.2).
...
In the frame header the size descriptor is followed by two flags bytes. These flags are described in [section 3.3.1](https://id3.org/id3v2.3.0#sec3.3.1).

There is no fixed order of the frames' appearance in the tag, although it is desired that the frames are arranged in order of significance concerning the recognition of the file. An example of such order: [UFID](https://id3.org/id3v2.3.0#sec4.1), [TIT2](https://id3.org/id3v2.3.0#TIT2), [MCDI](https://id3.org/id3v2.3.0#sec4.5), [TRCK](https://id3.org/id3v2.3.0#TRCK) ...

A tag must contain at least one frame. A frame must be at least 1 byte big, excluding the header.

If nothing else is said a string is represented as [ISO-8859-1](https://id3.org/id3v2.3.0#ISO-8859-1) characters in the range $20 - $FF. Such strings are represented as <text string>, or <full text string> if newlines are allowed, in the frame descriptions. All [Unicode](https://id3.org/id3v2.3.0#UNICODE) strings use 16-bit unicode 2.0 (ISO/IEC 10646-1:1993, UCS-2). Unicode strings must begin with the Unicode BOM ($FF FE or $FE FF) to identify the byte order.

All numeric strings and [URLs](https://id3.org/id3v2.3.0#URL) are always encoded as [ISO-8859-1](https://id3.org/id3v2.3.0#ISO-8859-1). Terminated strings are terminated with $00 if encoded with [ISO-8859-1](https://id3.org/id3v2.3.0#ISO-8859-1) and $00 00 if encoded as unicode. If nothing else is said newline character is forbidden. In [ISO-8859-1](https://id3.org/id3v2.3.0#ISO-8859-1) a new line is represented, when allowed, with $0A only. Frames that allow different types of text encoding have a text encoding description byte directly after the frame size. If [ISO-8859-1](https://id3.org/id3v2.3.0#ISO-8859-1) is used this byte should be $00, if Unicode is used it should be $01. Strings dependent on encoding is represented as <text string according to encoding>, or <full text string according to encoding> if newlines are allowed. Any empty [Unicode](https://id3.org/id3v2.3.0#UNICODE) strings which are NULL-terminated may have the Unicode BOM followed by a Unicode NULL ($FF FE 00 00 or $FE FF 00 00).

The three byte language field is used to describe the language of the frame's content, according to [ISO-639-2](https://id3.org/id3v2.3.0#ISO-639-2).

All [URLs](https://id3.org/id3v2.3.0#URL) may be relative, e.g. "picture.png", "../doc.txt".
...
This flag indicates wether or not the frame is enrypted. If set one byte indicating with which method it was encrypted will be appended to the frame header. See [section 4.26](https://id3.org/id3v2.3.0#sec4.26). for more information about encryption method registration.
...
3.  Preserved if tag is altered, discarded if file is altered.
    *   [AENC](https://id3.org/id3v2.3.0#sec4.21), [ETCO](https://id3.org/id3v2.3.0#sec4.6), [EQUA](https://id3.org/id3v2.3.0#sec4.13), [MLLT](https://id3.org/id3v2.3.0#sec4.7), [POSS](https://id3.org/id3v2.3.0#sec4.22), [SYLT](https://id3.org/id3v2.3.0#sec4.10), [SYTC](https://id3.org/id3v2.3.0#sec4.8), [RVAD](https://id3.org/id3v2.3.0#sec4.12), [TENC](https://id3.org/id3v2.3.0#TENC), [TLEN](https://id3.org/id3v2.3.0#TLEN), [TSIZ](https://id3.org/id3v2.3.0#TSIZ)
...
\[CDDB\] Compact Disc Data Base [http://www.cddb.com](http://www.cddb.com/)  
  
\[ID3v2\] Martin Nilsson, ["ID3v2 informal standard".](https://id3.org/id3v2-00)  
...
\[JFIF\] JPEG File Interchange Format, version 1.02, [http://www.w3.org/Graphics/JPEG/jfif.txt](http://www.w3.org/Graphics/JPEG/jfif.txt)  
  
\[MIME\] Freed, N. and N. Borenstein, "Multipurpose Internet Mail Extensions (MIME) Part One: Format of Internet Message Bodies", RFC 2045, November 1996., [ftp://ftp.isi.edu/in-notes/rfc2045.txt](ftp://ftp.isi.edu/in-notes/rfc2045.txt)  
...
\[PNG\] Portable Network Graphics, version 1.0, [http://www.w3.org/TR/REC-png-multi.html](http://www.w3.org/TR/REC-png-multi.html)  
  
\[UNICODE\] ISO/IEC 10646-1:1993. Universal Multiple-Octet Coded Character Set (UCS), Part 1: Architecture and Basic Multilingual Plane. Technical committee / subcommittee: JTC 1 / SC 2, [http://www.unicode.org](http://www.unicode.org/)  
  
\[URL\] T. Berners-Lee, L. Masinter & M. [McCahill](https://id3.org/McCahill), "Uniform Resource Locators (URL).", RFC 1738, December 1994., [ftp://ftp.isi.edu/in-notes/rfc1738.txt](ftp://ftp.isi.edu/in-notes/rfc1738.txt)  
  
\[ZLIB\] P. Deutsch, Aladdin Enterprises & J-L. Gailly, "ZLIB Compressed Data Format Specification version 3.3", RFC 1950, May 1996., url:[ftp://ftp.isi.edu/in-notes/rfc1950.txt](ftp://ftp.isi.edu/in-notes/rfc1950.txt)  
