...
if ! source "$SCRIPT_DIR/musiclib_db.sh" 2>/dev/null; then
    echo '{"error":"musiclib_db.sh not found","script":"musiclib_rate.sh","code":2,"context":{"expected_path":"'"$SCRIPT_DIR/musiclib_db.sh"'"},"timestamp":"'$(date -u +"%Y-%m-%dT%H:%M:%SZ")'"}' >&2
    exit 2
fi
if ! source "$SCRIPT_DIR/musiclib_player_utils.sh" 2>/dev/null; then
    echo '{"error":"musiclib_player_utils.sh not found","script":"musiclib_rate.sh","code":2,"context":{"expected_path":"'"$SCRIPT_DIR/musiclib_player_utils.sh"'"},"timestamp":"'$(date -u +"%Y-%m-%dT%H:%M:%SZ")'"}' >&2
    exit 2
fi
